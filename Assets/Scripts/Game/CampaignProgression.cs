...
namespace Game
{
    public class CampaignProgression
    {
        ...
        {
            foreach (var config in mission.MissionsToBlockTemporarily)
            {
                var missionDefinition = MissionsStorage.GetMissionDefinition(config.Id);
                if (missionDefinition.GetMissionState() != MissionState.Completed)
                    MissionsStorage.SetMissionState(config, state);
            }
        }
    }
}